...
export class Particles {
  ...
  emit(p, v, { color = [0.25, 0.01, 0.005, 0.95], life = 1.2, size = 0.05, drag = 0.5, grav = 9.8 } = {}) {
  ...
  update(dt) {
    for (let i = 0; i < this.n; i++) {
      ...
      if (this.pos[i * 3 + 1] < 0.02 && this.grav[i] > 0) { this.pos[i * 3 + 1] = 0.02; this.vel[i * 3] = this.vel[i * 3 + 1] = this.vel[i * 3 + 2] = 0; }
...
export class Decals {
  ...
  add(p, size = 1) {
    const m = new THREE.Mesh(new THREE.PlaneGeometry(size, size).rotateX(-Math.PI / 2), this.mat);
    m.position.copy(p).add(new THREE.Vector3(0, 0.03, 0)); m.rotation.y = Math.random() * 6.28; m.receiveShadow = true;
    this.scene.add(m);
  }
}
